...
# Otomatik Yenileme (Arka planda 15 saniyede bir verileri ve saati tazeler)
try:
...
# Tüm Orijinal CSS Düzenlemeleri
st.markdown("""
    <style>
    .stApp {
...
# 📍 1. SOL SÜTUN (BIST Endeksleri - Kararlı History Yöntemiyle)
with top_col1:
...
# 📍 2. ORTA SÜTUN (Net Türkiye Saati ve Piyasa Durumu)
with top_col2:
    try:
        import pytz
    ...
    <div class="time-status-box">
        <span style="color:#a3a3a3; font-weight:bold; font-size:12px;">⏱ shrink SON GÜNCELLEME (TSİ)</span><br>
...
# --- 🔥 ORİJİNAL HİSSE MOTORU GERİ YÜKLENDİ 🔥 ---
...
if hisse_input:
    ...
    try:
        ...
        if df.empty or len(df) < 20:
            ...
            # --- 🔮 TEKNİK HESAPLAMALAR GERİ GELDİ 🔮 ---
            ...
            # --- 3. ADIM: MUHTEŞEM MUM GRAFİĞİ (STRATEJİ HARİTASI) ---
            st.markdown("---")
            ...
            fig.update_layout(xaxis_rangeslider_visible=False, xaxis=dict(type='category', tickangle=-45), hovermode="x unified", template="plotly_dark", paper_bgcolor='#1e1e24', plot_bgcolor='#1e1e24', margin=dict(l=20, r=150, t=20, b=20), height=550, showlegend=False)
            ...
            # --- 5. ADIM: EKSİKSİZ VERİ TERMİNALİ (BİLANÇO VE TEKNİK TABLOLAR) ---
            st.markdown("---")
            ...
            # ÇEKMECE 2: Teknik İndikatörler ve Ortalamalar Paneli (RSI, MACD, MA TAM KADRO)
            ...
            # --- 6. ADIM: YAPAY ZEKA ANALİZ NOTLARI ---
        ...
        st.error("Hisse analiz verileri yüklenirken bir sorun yaşandı.")